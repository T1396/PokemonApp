...
class HomeDetailFragment : Fragment() {
    ...
    private fun showPokemonFormsDialog(formData: List<PokemonForList>, pokemonName: String) {
        val inflater = requireActivity().layoutInflater
        val view = inflater.inflate(R.layout.popup_pokemon_list, null)
        val rv = view.findViewById<RecyclerView>(R.id.rvPokemovelist)
        rv?.layoutManager = LinearLayoutManager(requireContext())

        val dialog = MaterialAlertDialogBuilder(requireContext()).apply {
            setTitle(getString(R.string.pokemon_forms, pokemonName))
            setView(view)
            setPositiveButton(android.R.string.ok, null)
        }.create()
        val adapter = PokeListAdapter { pokemonId ->
            showConfirmationDialog(pokemonId, dialog)
        }
        rv?.adapter = adapter
        adapter.submitList(formData)
        dialog.show()
    }

    private fun showConfirmationDialog(pokemonId: Int, dialog: AlertDialog) {
        showConfirmationDialog(onConfirm = {
            pokeViewModel.getSinglePokemonData(
                ...
                dialog.dismiss() // dismissed the "parent" dialog
            } // no callback needed
        }
        ) {}
    }
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        val languageNames = pokeViewModel.languageNames.value.orEmpty()
        val versionNames = pokeViewModel.versionNames.value.orEmpty()



        pokeViewModel.clickedPokemon.observe(viewLifecycleOwner) { pkData ->
            ...
            if (pkData.formData.isNotEmpty()) {
                pokeViewModel.loadFormDetails { pokemonFormList ->
                    ...
                    binding.ibFormInfos.setOnClickListener {
                        showPokemonFormsDialog(pokemonFormList, translatedName ?: "Error")
                    }
                }

            }

            sheetBinding.tvPokemonName.text = translatedName ?: "Error"
    ...
    private fun onVersionSelected(versionId: Int, adapter: PokedexEntryAdapter) {
        // Finden Sie die Position des Elements, das der ausgewählten Version entspricht
        val position =
            adapter.getPositionOfVersion(versionId)
        if (position != -1) {
            // Scrollen Sie zu dem Element in der RecyclerView
    ...
    private fun loadTypeCardview(
        ...
        // if typeId is null pokemon doesnt have second type
        if (typeId == null) {
            cardView.visibility = View.INVISIBLE
            return
        }
        val typeColorMap = pokeViewModel.getTypeColorMap()
    ...
    private fun fillRadarChart(
        ...
        // Konfiguriere die x-Achse (Kategorien)
        ...
        // Konfiguriere die y-Achse (Wertebereich)
